...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 14 21:21:06 2023
...
def enlarge_image():
    ...
    resized_image = image.resize((1000, 2000))
    tk_image = ImageTk.PhotoImage(resized_image)
    image_label = tk.Label(root, image=tk_image)
    image_label.pack()
    
   #Resize the Image using resize method
    resized_image= tk.img.resize((1000,2500), Image.ANTIALIAS)
    canvas.create_image(10,10, anchor='NW', image=resized_image)

def rotate_image():
    image = Image.open(file_path)
    image_rot = image.rotate(90)
    photo_rot = ImageTk.PhotoImage(image_rot)
    
    image_label = tk.Label(root, image=tk_image)
    image_label.pack()
    image_rot = image.rotate(90)
    canvas.create_image(10,10, anchor='NW', image=photo_rot)
...
pen_size_1 = tk.Radiobutton(
    pen_size_frame, text="Small", value=3, command=lambda: change_size(3), bg="black")
pen_size_1.pack(side="left")

pen_size_2 = tk.Radiobutton(
    pen_size_frame, text="Medium", value=5, command=lambda: change_size(5), bg="black")
pen_size_2.pack(side="left")
pen_size_2.select()

pen_size_3 = tk.Radiobutton(
    pen_size_frame, text="Large", value=7, command=lambda: change_size(7), bg="black")
...
enlarge_button = tk.Button(left_frame, text="Enlarge", command=enlarge_image, bg="#FF9797")
enlarge_button.pack()

rotate_button = tk.Button(left_frame, text="Rotate", command=rotate_image)
rotate_button.pack()
